...
namespace gameswf
{
		...
		m_is_open(true)
	{
		...
		m_dev = SDL_OpenAudioDevice(NULL, 0, &m_audioSpec, &m_audioHave, SDL_AUDIO_ALLOW_ANY_CHANGE);
		...
		if(m_audioSpec.format != m_audioHave.format){
			log_error("audio format not same");
		}
	...
	SDL_sound_handler::~SDL_sound_handler()
	{
		...
		if (m_is_open) SDL_CloseAudioDevice(m_dev);
		m_sound.clear();
	}
