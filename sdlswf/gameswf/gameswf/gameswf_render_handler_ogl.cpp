...
// XPM
static const char *s_line_image[] = {
	// width height num_colors chars_per_pixel
	"    32    32        3            1",
	// colors
	"X c #000000",
...
struct sdl_cursor_handler {
	...
	void init_line_cursor()
	{
		...
		sscanf(s_line_image[4 + row], "%d,%d", &hot_x, &hot_y);
		
		m_line_cursor = SDL_CreateCursor(data, mask, 32, 32, hot_x, hot_y);
	}
	...
	void set_cursor(gameswf::render_handler::cursor_type cursor)
	{
		...
		switch (cursor)
		{
			...
			case gameswf::render_handler::ACTIVE_CURSOR:
				...
				break;
			case gameswf::render_handler::LINE_CURSOR:
				if (m_active_cursor)
				{
					SDL_SetCursor(m_line_cursor);
				}
				break;
