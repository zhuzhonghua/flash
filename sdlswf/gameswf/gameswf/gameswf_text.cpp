...
//				* pixel_scale;
	...
	{ 
		...
		if (m_has_focus) 
		{ 
			Uint32 tick = tu_timer::get_ticks();
			if(tick - m_delta_time > 600){
				m_delta_time = tick;
				m_bcursor = !m_bcursor;
			}
	...
	void edit_text_character::set_cursor()
	{
		if(m_text_glyph_records.size() <= 0) return;
		...
		const tu_string* textPtrToUse = &m_text;
		...
		float cur_x = m_def->m_rect.m_x_min;
		...
		{
			cur_x += rec.m_style.m_font->get_kerning_adjustment(last_code, (int) code) * rec.m_style.m_scale;
			last_code = (int) code;

			// find glyph
			glyph	g;
			rec.m_style.m_font->get_glyph(&g, (Uint16) code, fontsize);

			// for device font set xscale = yscale
			g.m_glyph_advance *= yscale * rec.m_style.m_scale / xscale;
			g.m_fontsize = fontsize;

			cur_x += g.m_glyph_advance;

			character_idx++;

			// check the cursor left or right
			// todo: multi line
			// currently single line
			// left half
			if(m_xcursor2 < cur_x - g.m_glyph_advance/2){
				break;
			}
			else{
				m_cursor = character_idx;
			}
		}
	}
		...
		case event_id::PRESS:
			{
				// todo: optimize
				if(m_has_focus == true){
					// set m_cursor according to x and y
					set_cursor();
					format_text();
				}
				break;
	...
	bool edit_text_character::can_handle_mouse_select_event()
	{
		return m_def->m_readonly == false;
	}
...
//		printf("%s\n", m_text.c_str());
	...
	void	edit_text_character::format_plain_text(const tu_string& text, text_glyph_record&	rec)
	{
		...
		m_y_start = rec.m_style.m_leading;
		m_ycursor = m_y_start;
		...
		while (Uint32 code = utf8::decode_next_unicode_character(&text_ptr))
		{
			...
			if (code == 13 || code == 10)
			{
				...
				m_y_start += rec.m_style.m_text_height;
