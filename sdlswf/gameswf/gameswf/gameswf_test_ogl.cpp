...
static gameswf::key::code	translate_key(SDL_Keycode key)
// For forwarding SDL key events to gameswf.
{
	...
	{
		c = (gameswf::key::code) ((key - SDLK_KP_0) + gameswf::key::KP_0);
	}
	else
	{
...
// For handling notification callbacks from ActionScript.
{
	...
	if (stricmp(command, "notify_keypress") == 0)
	{
		...
		for (int i = 0, n = strlen(args); i < n; i++)
		{
			...
			SDL_Keycode key = static_cast<SDL_Keycode>(tolower(args[i]));
