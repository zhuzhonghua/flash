...
//			register_tag_loader(63, define_font_info_loader);
			...
			register_tag_loader(78, define_scaling_grid_loader);	// DefineScalingGrid - Flash 8			
...
*/
	...
	void	define_bits_jpeg2_loader(stream* in, int tag_type, movie_definition_sub* m)
	{
		...
		if (m->get_create_bitmaps() == DO_LOAD_BITMAPS)
		{
			...
			// TagLater:
			StreamAdapter sa(in->get_underlying_stream(), in->get_tag_end_position());
			image::rgb* im = image::read_swf_jpeg2(&sa);
			bi = render::create_bitmap_info_rgb(im);
			delete im;
#else
			log_error("gameswf is not linked to jpeglib -- can't load jpeg image data!\n");
	...
	void	define_scaling_grid_loader(stream* in, int tag_type, movie_definition_sub* m)
	{
		assert(tag_type == 78);

		// if flags == 1 then parse only
		Uint16 id = in->read_u16();
		IF_VERBOSE_PARSE(log_msg("\n defineScalingGrid tag loader, id = '%d'\n", id));
	...
	// this tag defines characteristics of the SWF file (Flash 8)
	{
		...
		m->m_use_network =  attr & 0x01 ? true : false;
	}
