...
// TagLater:
static int adapter_read_func(void* dst, int bytes, void* appdata)
// Return the number of bytes actually read.  EOF or an error would
// cause that to not be equal to "bytes".
{
	assert(appdata);
	assert(dst);
	StreamAdapter* sa = (StreamAdapter*)appdata;
	int bytesLeft = sa->endPos - sa->currPos;
	if (bytesLeft < bytes) {
		if (!bytesLeft) return 0;		
		bytes = bytesLeft;
	}
	int actuallyRead = sa->s->read_bytes(dst, bytes);
	sa->currPos += actuallyRead;
	return actuallyRead;
}

StreamAdapter::StreamAdapter(tu_file* underS, int maxPos) :
tu_file(this, 
adapter_read_func, 
NULL, 
NULL,
