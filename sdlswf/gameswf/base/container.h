...
class hash : public hash_map<T, U, hash_functor>
{
	typedef hash_map<T, U, hash_functor> BaseClass;
	typedef hash_map<T, U, hash_functor>::const_iterator const_iterator;
public:
	...
	{
		assert(BaseClass::find(key) == BaseClass::end());
		(*this)[key] = value;
	}

	bool	is_empty() const { return BaseClass::empty(); }
	...
	{
		const_iterator	it = BaseClass::find(key);
		if (it != end())
		{
